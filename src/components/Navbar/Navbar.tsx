...
function Navbar() {
    ...
    useEffect(() => {
        if (activeLinkBackgroundRef.current) {
            activeLinkBackgroundRef.current.style.left = `${
                activeLinkRef.current?.offsetLeft || 0
            }px`;
            activeLinkBackgroundRef.current.style.width = `${
                activeLinkRef.current?.offsetWidth || 0
            }px`;
            if (!activeLinkBackgroundRef.current.style.height) {
                activeLinkBackgroundRef.current.style.height = `${
                    ...
                    <>
                        <Sun
                            className={`absolute h-2/3 transition-[fill] fill-yellow-300 ${
                                isActive && 'animate-spin-5s'
                            } ${isPending && 'animate-spin-3s'}`}
                        />
                        <CloudLeft
                            className={`absolute z-[1] h-2/3 transition-transform fill-slate-400 w-fit translate-x-2 translate-y-1 delay-100 duration-[275px] ${
                                isActive && 'translate-x-[1.625rem]'
                            } group-hover:translate-x-6`}
                        />
                        <CloudRight
                            className={`absolute z-[2] h-2/3 transition-transform fill-slate-300 w-fit -translate-x-2 translate-y-1 scale-95 delay-200 duration-[375ms] shadow-smp ${
                                isActive && '-translate-x-[1.625rem]'
                            } group-hover:-translate-x-6`}
                        />
                    </>
                ...
                <NavLink
                    ...
                    className={({ isActive, isPending }) =>
                        `relative z-[1] px-2 py-1 rounded-lg transition-colors delay-200 underline decoration-transparent ${
                            !isActive && 'hover:decoration-white'
                        } ${isActive && 'text-black'} ${
                            isPending && 'pointer-events-none bg-gray-500'
                        }`
                    }
                >
                    Current
